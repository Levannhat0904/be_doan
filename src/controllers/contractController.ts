...
export const updateContract = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const newStartDate = startDate ? new Date(startDate) : existingContract[0].startDate;
      const newEndDate = endDate ? new Date(endDate) : existingContract[0].endDate;
      const newDepositAmount = depositAmount || existingContract[0].depositAmount;
      ...
      if (isRoomChanged && existingContract[0].status === 'active') {
        ...
        if (updatedNewRoomInfo.length && updatedNewRoomInfo[0].currentOccupancy >= updatedNewRoomInfo[0].capacity) {
          await pool.query(
            `UPDATE rooms SET status = 'full' WHERE id = ?`,
            [newRoomId]
          );
      ...
      else if (status === 'terminated' && existingContract[0].status !== 'terminated') {
        // Update room occupancy
        await pool.query(
          `UPDATE rooms SET currentOccupancy = currentOccupancy - 1 WHERE id = ?`,
          [existingContract[0].roomId]
      ...
      if (req.user?.id && details.length > 0) {
        ...
        if (status && status !== existingContract[0].status) {
          ...
          if (depositAmount && depositAmount !== existingContract[0].depositAmount) {
            changes.push(`tiền đặt cọc: ${depositAmount.toLocaleString('vi-VN')} VNĐ`);
          }

          if (monthlyFee && monthlyFee !== existingContract[0].monthlyFee) {
            changes.push(`phí hàng tháng: ${monthlyFee.toLocaleString('vi-VN')} VNĐ`);
          }

          changesDescription = changes.length > 0
